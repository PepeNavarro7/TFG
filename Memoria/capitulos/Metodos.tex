...
\begin{algorithm}[H] %Le ponemos la H para que se coloque entre los dos párrafos, y no se mueva libremente
	\begin{algorithmic}
		...
		\While{$t_n < t_f$}
		\State $y_k \gets AdamsBashford_k(y_{k-1},f_{n})$ \Comment{Aplicar orden correspondiente}
		\State $f_k \gets feval(y_k)$
		\State $swap(y_k,y_{k-1})$
		\State $swap(f_n,f_{n-1})$ \Comment{$0 \leq n < k$}
		\State $t_n \gets t_n + h$
		\EndWhile
...
\begin{algorithm}[H] %Le ponemos la H para que se coloque entre los dos párrafos, y no se mueva libremente
	\begin{algorithmic}
		...
		\State $t_n \gets t_0 + (k-1) \cdot h$ 
		\While{$t_n < t_f$}
		\State $y_{pred} \gets AdamsBashford_k(y_{k-1},f_{n})$ \Comment{Aplicar orden k}
		\State $f_{pred} \gets feval(y_{pred})$
		\State $y_{corr} \gets AdamsMoulton_k(y_{k-1},f_{pred})$ \Comment{Aplicar orden k}
		\State $f_{corr} \gets feval(y_{corr})$
		\State $y_k \gets AdamsMoulton_k(y_{k-1},f_{corr})$ \Comment{Aplicar orden k}
		\State $f_k \gets feval(y_k)$
		\State $swap(y_k,y_{k-1})$
		\State $swap(f_n,f_{n-1})$ \Comment{$0 \leq n < k$}
		\State $t_n \gets t_n + h$
