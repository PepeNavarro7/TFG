...
La expresión general de un método de Adams-Bashford de k pasos es:\cite{Molero2007}

\begin{equation}
	\label{eq:ExpresionGeneralAdamsBashford}
	\begin{split}
...
Aplicando la anterior ecuación \ref{eq:ExpresionGeneralAdamsBashford}, obtendríamos los distintos órdenes con los que trabajaremos. Al igual que con Runge-Kutta, el Adams-Bashford de orden 1 coincide con Euler explícito (ecuación \ref{eq:EulerExplicito}). De igual manera, obtendríamos Adams-Bashford de 2 pasos:

\begin{equation}
	\label{eq:AdamsBashford2}
...
\begin{algorithm}[H] %Le ponemos la H para que se coloque entre los dos párrafos, y no se mueva libremente
	\begin{algorithmic}
		...
		\While{$t_n < t_f$}
		\State $Y_k \gets AdamsBashford\text{-}K(Y_{k-1}\text{, }F_{n})$ \Comment{$0 \leq n < k$}
	...
	\end{algorithmic}
	\caption{Adams-Bashford general}
	\label{alg:AdamsBashford}
\end{algorithm}
...
\begin{algorithm}[H] %Le ponemos la H para que se coloque entre los dos párrafos, y no se mueva libremente
	\begin{algorithmic}
		...
		\While{$t_n < t_f$}
			...
			\State $Y_{pred} \gets AdamsBashford\text{-}K( Y_{k-1} \text{, } F_{k-1} )$ \Comment{Aplicar orden k}
		...
		\State $Y_f \gets Y_{k-1}$
	\end{algorithmic}
	\caption{Adams-Bashford-Moulton general}
	\label{alg:AdamsBashfordMoulton}
\end{algorithm}
