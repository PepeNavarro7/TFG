...
\begin{algorithm}[H] %Le ponemos la H para que se coloque entre los dos párrafos, y no se mueva libremente
	\begin{algorithmic}
		...
		\While{$t_n < t_f$}
		\State $y_k \gets y_{k-1} + h \cdot [AB_k]$ \Comment{Aplicar orden correspondiente}
	...
	\end{algorithmic}
	\caption{Adams-Bashford generalizado}
	\label{alg:AdamsBashford4}
\end{algorithm}

La idea detrás del algoritmo es, tras el arranque de los k-1 vectores ``$y$'', y la evaluación de los k vectores ``$f$'', donde k es igual al orden de A-B que estemos aplicando, entrar en el bucle iterativo. En él, iteraremos sobre 2 vectores ``$y$'', a saber, $y_k$ y $y_{k-1}$, y sobre k+1 vectores ``$f$''. 
