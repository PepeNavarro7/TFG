...
\begin{algorithm}[H] %Le ponemos la H para que se coloque entre los dos párrafos, y no se mueva libremente
	...
	\begin{algorithmic}
		...
		\State $Y_{n+1} \gets Y_n + h/6 \cdot (K1 + 2 \cdot K2 + 2 \cdot K3 + K4)$
		\State $t_n += h$
		\EndWhile
	\end{algorithmic}
\end{algorithm}

Como podemos ver, la aplicación del algoritmo es relativamente simple, se trabaja con 4 vectores auxiliares en los que hacemos los cálculos intermedios, que luego se suman. Es en estas operaciones, las evaluaciones de cada miembro, las sumas, y las multiplicaciones de cada vector, en las que podremos entrar a trabajar el paralelismo para mejorar la eficiencia de nuestro  código.
