#ifndef ADAMS_MOULTON_I_CPP
#define ADAMS_MOULTON_I_CPP
...
void AdamsMoulton_i::aplicar(const Problema* problema, const double &t0, const double &tf, const double &h, const double* __restrict Y0, double* __restrict Yf) const {
    ...
    switch(get_orden()){ // Arrancamos los feval
        ...
        case 2:
            ...
            for (int i = 0; i < neqn; ++i)// Definimos el vector feval Fn0
        ...
        case 3:
            ...
            for (int i = 0; i < neqn; ++i){// Definimos los vectores feval Fn0 & Fn1
                Fn0[i] = problema->feval_i(t0,      Yn0, i); // f(tn0,Yn0) -> Fn0
        ...
        case 4:
            ...
            for (int i = 0; i < neqn; ++i){// Definimos los vectores feval Fn0, Fn1 & Fn2
                Fn0[i] = problema->feval_i(t0,          Yn0, i); // f(tn0,Yn0) -> Fn0
                Fn1[i] = problema->feval_i(t0+h_RK,     Yn1, i); // f(tn1,Yn1) -> Fn1
        ...
        case 5:
            ...
            for (int i = 0; i < neqn; ++i){// Definimos los vectores feval Fn0, Fn1, Fn2 & Fn3
                Fn0[i] = problema->feval_i(t0,          Yn0, i); // f(tn0,Yn0) -> Fn0
                Fn1[i] = problema->feval_i(t0+h_RK,     Yn1, i); // f(tn1,Yn1) -> Fn1
                Fn2[i] = problema->feval_i(t0+h_RK*2.0, Yn2, i); // f(tn2,Yn2) -> Fn2
    ...
    switch(get_orden()){
        ...
        #pragma omp parallel default(none) shared(problema, t0, tf, h, neqn, Y_AB, F_AB, Y_AM, F_AM, Yn0, Fn0, Yn1, Fn1)
        {
            for (double tn=t0; tn<tf; tn+=h){
                ...
                #pragma omp for schedule(static) // Obtenemos el feval de Y_AB
                ...
                #pragma omp for schedule(static) // Obtenemos el feval de Y_AM
                for (int i = 0; i < neqn; ++i){
                    ...
                    Yn1[i] = Yn0[i] + h * F_AM[i];// Yn1 = Yn0 + h * F_AM
                }

                #pragma omp for schedule(static) // Calculamos el nuevo Fn1 usando el Yn1 recién creado
                for (int i = 0; i < neqn; ++i)
                    Fn1[i] = problema->feval_i(tn+h, Yn1, i); // f(tn1,Yn1) -> Fn1

                #pragma omp single // Ahora que tenemos Yn1 & Fn1, convertimos los n en n-1 para hacer la siguiente iteracion
                {
        ...
        #pragma omp parallel default(none) shared(problema, t0, tf, h, h2, neqn, Y_AB, F_AB, Y_AM, F_AM, Yn0, Fn0, Yn1, Fn1)
        {
            for (double tn=t0; tn<tf; tn+=h){
                ...
                for (int i = 0; i < neqn; ++i){// Obtenemos el feval de Y_AB
                    F_AB[i] = problema->feval_i(tn+h, Y_AB, i); // f(tn1, Y_AB) -> F_AB

                    // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn1 -> Y_AM
                    Y_AM[i] = Yn0[i] + h2 * (F_AB[i] + Fn0[i]);// Y_AM = Yn0 + h/2 * (F_AB + Fn0)
                }

                #pragma omp for schedule(static)
                for (int i = 0; i < neqn; ++i){// Obtenemos el feval de Y_AM
                    F_AM[i] = problema->feval_i(tn+h, Y_AM, i); // f(tn1, Y_AM) -> F_AM

                    // Ahora que tenemos una 2ª aproximación de Yn1, volvemos a aplicar A-M con ella, para obtener el Yn1 definitivo
                    Yn1[i] = Yn0[i] + h2 * (F_AM[i] + Fn0[i]); // Yn1 = Yn0 + h/2 * (F_AM + Fn0)
                }
        ...
        #pragma omp parallel default(none) shared(problema, t0, tf, h, h_RK, h2, h12, neqn, Y_AB, F_AB, Y_AM, F_AM, Fn0, Yn1, Fn1, Yn2, Fn2)
        {
            for (double tn=t0+h_RK; tn<tf; tn+=h){
                ...
                for (int i = 0; i < neqn; ++i){ // Obtenemos el feval de Y_AB
                    ...
                    Y_AM[i] = Yn1[i] + h12 * (5*F_AB[i] + 8.0*Fn1[i] - 1*Fn0[i]); // Y_AM = Yn1 + h/12 * (5F_AB + 8Fn1 - 1Fn0)
                }                    
                ...
                for (int i = 0; i < neqn; ++i){ // Obtenemos el feval de Y_AM
                    ...
                    Yn2[i] = Yn1[i] + h12 * (5*F_AM[i] + 8.0*Fn1[i] - 1*Fn0[i]);// Yn2 = Yn1 + h/12 * (5Fn_AM + 8Fn1 - 1Fn0)
                }

                #pragma omp for schedule(static) // Calculamos el nuevo Fn2 usando el Yn2 recién creado
                for (int i = 0; i < neqn; ++i)
                    Fn2[i] = problema->feval_i(tn+h, Yn2, i); // f(tn2,Yn2) -> Fn2
                    

                #pragma omp single // Ahora que tenemos Yn2 & Fn2, convertimos todos los n en n-1 para hacer la siguiente iteracion
                {
        ...
        #pragma omp parallel default(none) shared(problema, t0, tf, h, h_RK, h12, h24, neqn, Y_AB, F_AB, Y_AM, F_AM, Fn0, Fn1, Yn2, Fn2, Yn3, Fn3)
        {
            for (double tn=t0+h_RK*2.0; tn<tf; tn+=h){
                ...
                for (int i = 0; i < neqn; ++i){// Obtenemos el feval de Y_AB
                    ...
                    Y_AM[i] = Yn2[i] + h24 * (9*F_AB[i] + 19.0*Fn2[i] - 5.0*Fn1[i] + 1*Fn0[i]);// Y_AM = Yn2 + h/24 * (9F_AB + 19Fn2 - 5Fn1 + 1Fn0)
                ...
                for (int i = 0; i < neqn; ++i){// Obtenemos el feval de Y_AM
                    ...
                    Yn3[i] = Yn2[i] + h24 * (9.0*F_AM[i] + 19.0*Fn2[i] - 5.0*Fn1[i] + 1*Fn0[i]); // Yn3 = Yn2 + h/24 * (9F_AM + 19Fn2 - 5Fn1 + 1Fn0)
                }
                    
                #pragma omp for schedule(static)// Calculamos el nuevo Fn3 usando el Yn3 recién creado
                for (int i = 0; i < neqn; ++i)
                    Fn3[i] = problema->feval_i(tn+h, Yn3, i); // f(tn3,Yn3) -> Fn3

                #pragma omp single // Ahora que tenemos Yn3 & Fn3, convertimos todos los n en n-1 para hacer la siguiente iteracion
                {
        ...
        #pragma omp parallel default(none) shared(problema, t0, tf, h, h_RK, h24, h720, neqn, Y_AB, F_AB, Y_AM, F_AM, Fn0, Fn1, Fn2, Yn3, Fn3, Yn4, Fn4)
        {
            for (double tn=t0+h_RK*3.0; tn<tf; tn+=h){
                ...
                for (int i = 0; i < neqn; ++i){ // Obtenemos el feval de Y_AB
                    ...
                    Y_AM[i] = Yn3[i] + h720 * (251.0*F_AB[i] + 646.0*Fn3[i] - 264.0*Fn2[i] + 106.0*Fn1[i] - 19.0*Fn0[i]); // Y_AM = Yn3 + h/720 * (251F_AB + 646Fn3 - 264Fn2 + 106Fn1 - 19Fn0)
                }
                ...
                for (int i = 0; i < neqn; ++i){// Obtenemos el feval de Y_AM
                    ...
                    Yn4[i] = Yn3[i] + h720 * (251.0*F_AM[i] + 646.0*Fn3[i] - 264.0*Fn2[i] + 106.0*Fn1[i] - 19.0*Fn0[i]); // Yn4 = Yn3 + h/720 * (251F_AM + 646Fn3 - 264Fn2 + 106Fn1 - 19Fn0)
                }
                    
                #pragma omp for schedule(static) // Calculamos el nuevo Fn4 usando el Yn4 recién creado
                for (int i = 0; i < neqn; ++i)
                    Fn4[i] = problema->feval_i(tn+h, Yn4, i); // f(tn4,Yn4) -> Fn4
                    

                #pragma omp single // Ahora que tenemos Yn4 & Fn4, convertimos todos los n en n-1 para hacer la siguiente iteracion
                {
