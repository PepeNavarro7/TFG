...
private:
    ...
    const double dtx_2_inv, // Spatial step doubled
                 dtx_sq_inv, // Spatial step squared
...
public:
    ...
    simpleadvdiff1d(const int &nx_points): 
        ...
        nx(nx_points), dtx_2_inv( 1.0/(2.0*get_dtx()) ), dtx_sq_inv( (1.0/(get_dtx()*get_dtx()) ) ) {};
    ...
    inline void archivo (const string &filename, const double* __restrict Y) const override { archivo1(filename,Y); };
};
#endif