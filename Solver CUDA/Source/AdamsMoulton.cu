...
void AdamsMoulton::aplicar(Problema* problema, const double &t0, const double &tf, const double &h, const double *Y0, double *Yf) const {
    ...
    cudaMalloc((void**)&Y_AB,NUM_BYTES);
    cudaMalloc((void**)&F_AB,NUM_BYTES);
    cudaMalloc((void**)&Y_AM,NUM_BYTES);
    cudaMalloc((void**)&F_AM,NUM_BYTES);
    ...
    switch(orden){
        case 1: // Yn+1 = Yn + h * Fn+1
            for (double tn=t0; tn<tf; tn+=h){
                ...
                kernel_sumatoriaAB1<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AB, Yn0, Fn0); // Y_AB = Yn0 + h * Fn0
                ...
                kernel_sumatoriaAM1<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AM, Yn0, F_AB);// Y_AM = Yn0 + h * F_AB
                ...
                kernel_sumatoriaAM1<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Yn1, Yn0, F_AM);
        ...
        case 2: // Yn+1 = Yn + h/2 * (Fn+1 + Fn)
            for (double tn=t0; tn<tf; tn+=h){
                ...
                // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn1 -> Y_AM
                kernel_sumatoriaAM2<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AM, Yn0, F_AB, Fn0);// Y_AM = Yn0 + h/2 * (F_AB + Fn0)

                // Obtenemos el feval de Y_AM
                problema->feval(tn+h, Y_AM, F_AM); // f(tn1, Y_AM) -> F_AM

                // Ahora que tenemos una 2ª aproximación de Yn1, volvemos a aplicar A-M con ella, para obtener el Yn1 definitivo
                kernel_sumatoriaAM2<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Yn1, Yn0, F_AM, Fn0); // Yn1 = Yn0 + h/2 * (F_AM + Fn0)
        ...
        case 3: // Yn+1 = Yn + h/12 * (5Fn+1 + 8Fn - 1Fn-1)
            for (double tn=t0+h_RK; tn<tf; tn+=h){
                ...
                kernel_sumatoriaAB2<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AB, Yn1, Fn1, Fn0); // Y_AB = Yn1 + h/2 * (3*Fn1 - Fn0)         
                ...
                kernel_sumatoriaAM3<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AM, Yn1, F_AB, Fn1, Fn0); // Y_AM = Yn1 + h/12 * (5F_AB + 8Fn1 - 1Fn0)
                ...
                kernel_sumatoriaAM3<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Yn2, Yn1, F_AM, Fn1, Fn0);// Yn2 = Yn1 + h/12 * (5Fn_AM + 8Fn1 - 1Fn0)
        ...
        case 4: // Yn+1 = Yn + h/24 * (9Fn+1 + 19Fn - 5Fn-1 + 1Fn-2)
            for (double tn=t0+h_RK*2.0; tn<tf; tn+=h){
                ...
                kernel_sumatoriaAB3<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AB, Yn2, Fn2, Fn1, Fn0);// Y_AB = Yn2 + h/12 * (23*Fn2 - 16*Fn1 + 5*Fn0)
                ...
                kernel_sumatoriaAM4<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AM, Yn2, F_AB, Fn2, Fn1, Fn0);// Y_AM = Yn2 + h/24 * (9F_AB + 19Fn2 - 5Fn1 + 1Fn0)
                ...
                kernel_sumatoriaAM4<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Yn3, Yn2, F_AM, Fn2, Fn1, Fn0);// Yn3 = Yn2 + h/24 * (9F_AM + 19Fn2 - 5Fn1 + 1Fn0)
        ...
        case 5: // Yn+1 = Yn + h/720 * (251Fn+1 + 646Fn - 264Fn-1 + 106Fn-2 - 19Fn-3)
            for (double tn=t0+h_RK*3.0; tn<tf; tn+=h){
                ...
                kernel_sumatoriaAB4<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AB, Yn3, Fn3, Fn2, Fn1, Fn0);// Y_AB = Yn3 + h/24 * (55Fn3 - 59Fn2 + 37Fn1 - 9Fn0)
                ...
                kernel_sumatoriaAM5<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Y_AM, Yn3, F_AB, Fn3, Fn2, Fn1, Fn0); // Y_AM = Yn3 + h/720 * (251F_AB + 646Fn3 - 264Fn2 + 106Fn1 - 19Fn0)
                ...
                kernel_sumatoriaAM5<<<NUM_BLOCKS,THREADSPERBLOCK>>>(Yn4, Yn3, F_AM, Fn3, Fn2, Fn1, Fn0); // Yn4 = Yn3 + h/720 * (251F_AM + 646Fn3 - 264Fn2 + 106Fn1 - 19Fn0)
