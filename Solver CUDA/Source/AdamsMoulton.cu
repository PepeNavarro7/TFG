...
__constant__ Params_AdamsMoulton cte_AM;
...
void AdamsMoulton::updateConstants(const int &neqn, const double &h) const {
    ...
    cudaMemcpyToSymbol(cte_AM, &aux, sizeof(Params_AdamsMoulton));
}
...
__global__ void kernel_sumatoriaAM1(double* __restrict__ Yn1, const double* __restrict__ Yn0, const double* __restrict__ Fn1){
    ...
    if (thread < cte_AM.neqn){ // Yn+1 = Yn + h * Fn+1
        Yn1[thread] = Yn0[thread] + cte_AM.h * Fn1[thread];
    }
}
...
__global__ void kernel_sumatoriaAM2(double* __restrict__ Yn1, const double* __restrict__ Yn0, const double* __restrict__ Fn1, const double* __restrict__ Fn0){
    const int thread = blockDim.x * blockIdx.x + threadIdx.x;
    if (thread < cte_AM.neqn){ // Yn+1 = Yn + h/2 * (Fn+1 + Fn)
        Yn1[thread] = Yn0[thread] + cte_AM.h2 * (Fn1[thread] + Fn0[thread]);
    }
}
...
__global__ void kernel_sumatoriaAM3(double* __restrict__ Yn2, const double* __restrict__ Yn1, const double* __restrict__ Fn2, const double* __restrict__ Fn1, const double* __restrict__ Fn0){
    const int thread = blockDim.x * blockIdx.x + threadIdx.x;
    if (thread < cte_AM.neqn){ // Yn+1 = Yn + h/12 * (5Fn+1 + 8Fn - 1Fn-1)
        Yn2[thread] = Yn1[thread] + cte_AM.h12 * (5.0*Fn2[thread] + 8.0*Fn1[thread] - Fn0[thread]);
    }
}
...
__global__ void kernel_sumatoriaAM4(double* __restrict__ Yn3, const double* __restrict__ Yn2, const double* __restrict__ Fn3, const double* __restrict__ Fn2, const double* __restrict__ Fn1, const double* __restrict__ Fn0){
    const int thread = blockDim.x * blockIdx.x + threadIdx.x;
    if (thread < cte_AM.neqn){ // Yn+1 = Yn + h/24 * (9Fn+1 + 19Fn - 5Fn-1 + 1Fn-2)
        Yn3[thread] = Yn2[thread] + cte_AM.h24 * (9.0*Fn3[thread] + 19.0*Fn2[thread] - 5.0*Fn1[thread] + Fn0[thread]);
    }
}
...
__global__ void kernel_sumatoriaAM5(double* __restrict__ Yn4, const double* __restrict__ Yn3, const double* __restrict__ Fn4, const double* __restrict__ Fn3, const double* __restrict__ Fn2, const double* __restrict__ Fn1, const double* __restrict__ Fn0){
    const int thread = blockDim.x * blockIdx.x + threadIdx.x;
    if (thread < cte_AM.neqn){ // Yn+1 = Yn + h/720 * (251Fn+1 + 646Fn - 264Fn-1 + 106Fn-2 - 19Fn-3)
        Yn4[thread] = Yn3[thread] + cte_AM.h720 * (251.0*Fn4[thread] + 646.0*Fn3[thread] - 264.0*Fn2[thread] + 106.0*Fn1[thread] - 19.0*Fn0[thread]);
    }
}
    ...
    switch(get_orden()){
        case 1: // Yn+1 = Yn + h * Fn+1
            for (double tn=t0; tn<tf; tn+=h){
                // Aplicamos Adams-Bashford para obtener una 1ª aproximación de Yn1 -> Y_AB & F_AB
                kernel_sumatoriaAB1<<<get_num_blocks(),get_tam_blocks()>>>(Y_AB, Yn0, Fn0); // Y_AB = Yn0 + h * Fn0
                problema->feval(tn+h, Y_AB, F_AB); // f(tn1, Y_AB) -> F_AB
                
                // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn1 -> Y_AM & F_AM
                kernel_sumatoriaAM1<<<get_num_blocks(),get_tam_blocks()>>>(Y_AM, Yn0, F_AB);// Y_AM = Yn0 + h * F_AB
                problema->feval(tn+h, Y_AM, F_AM); // f(tn1, Y_AM) -> F_AM

                // Ahora que tenemos una 2ª aproximación de Yn1, volvemos a aplicar A-M con ella, para obtener Yn1 & Fn1 
                kernel_sumatoriaAM1<<<get_num_blocks(),get_tam_blocks()>>>(Yn1, Yn0, F_AM);
                ...
                // Tras los cambios, Yn1, Fn1, Y_AB, F_AB, Y_AM & F_AM contienen basura y serán reescritos
        ...
        case 2: // Yn+1 = Yn + h/2 * (Fn+1 + Fn)
            for (double tn=t0; tn<tf; tn+=h){
                ...
                // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn1 -> Y_AM & F_AM
                kernel_sumatoriaAM2<<<get_num_blocks(),get_tam_blocks()>>>(Y_AM, Yn0, F_AB, Fn0);// Y_AM = Yn0 + h/2 * (F_AB + Fn0)
                problema->feval(tn+h, Y_AM, F_AM); // f(tn1, Y_AM) -> F_AM

                // Ahora que tenemos una 2ª aproximación de Yn1, volvemos a aplicar A-M con ella, para obtener Yn1 & Fn1
                kernel_sumatoriaAM2<<<get_num_blocks(),get_tam_blocks()>>>(Yn1, Yn0, F_AM, Fn0); // Yn1 = Yn0 + h/2 * (F_AM + Fn0)
                problema->feval(tn+h, Yn1, Fn1); // f(tn1,Yn1) -> Fn1
        ...
        case 3: // Yn+1 = Yn + h/12 * (5Fn+1 + 8Fn - 1Fn-1)
            for (double tn=t0+h_RK; tn<tf; tn+=h){
                //Aplicamos Adams-Bashford para obtener una 1ª aproximación de Yn2 -> Y_AB & F_AB
                kernel_sumatoriaAB2<<<get_num_blocks(),get_tam_blocks()>>>(Y_AB, Yn1, Fn1, Fn0); // Y_AB = Yn1 + h/2 * (3*Fn1 - Fn0)
                problema->feval(tn+h, Y_AB, F_AB); // f(tn2, Y_AB) -> F_AB
                
                // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn2 -> Y_AM & F_AM
                kernel_sumatoriaAM3<<<get_num_blocks(),get_tam_blocks()>>>(Y_AM, Yn1, F_AB, Fn1, Fn0); // Y_AM = Yn1 + h/12 * (5F_AB + 8Fn1 - 1Fn0)
                ...
                kernel_sumatoriaAM3<<<get_num_blocks(),get_tam_blocks()>>>(Yn2, Yn1, F_AM, Fn1, Fn0);// Yn2 = Yn1 + h/12 * (5Fn_AM + 8Fn1 - 1Fn0)
                problema->feval(tn+h, Yn2, Fn2); // f(tn2,Yn2) -> Fn2

                // Ahora que tenemos Yn2 & Fn2, convertimos n en n-1 para hacer la siguiente iteracion
                swap(Yn2, Yn1); // Yn2 -> Yn1
                swap(Fn1, Fn0); // Fn1 -> Fn0
                swap(Fn2, Fn1); // Fn2 -> Fn1
                // Tras los cambios, Yn2, Fn2, Y_AB, F_AB, Y_AM & F_AM contienen basura y serán reescritos                
            } // Fin del bucle for principal
        break;
        case 4: // Yn+1 = Yn + h/24 * (9Fn+1 + 19Fn - 5Fn-1 + 1Fn-2)
            for (double tn=t0+h_RK*2.0; tn<tf; tn+=h){
                // Aplicamos Adams-Bashford para obtener una 1ª aproximación de Yn3 -> Y_AB & F_AB
                kernel_sumatoriaAB3<<<get_num_blocks(),get_tam_blocks()>>>(Y_AB, Yn2, Fn2, Fn1, Fn0);// Y_AB = Yn2 + h/12 * (23*Fn2 - 16*Fn1 + 5*Fn0)
                problema->feval(tn+h, Y_AB, F_AB); // f(tn3, Y_AB) -> F_AB
                
                // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn3 -> Y_AM & F_AM
                kernel_sumatoriaAM4<<<get_num_blocks(),get_tam_blocks()>>>(Y_AM, Yn2, F_AB, Fn2, Fn1, Fn0);// Y_AM = Yn2 + h/24 * (9F_AB + 19Fn2 - 5Fn1 + 1Fn0)
                ...
                kernel_sumatoriaAM4<<<get_num_blocks(),get_tam_blocks()>>>(Yn3, Yn2, F_AM, Fn2, Fn1, Fn0);// Yn3 = Yn2 + h/24 * (9F_AM + 19Fn2 - 5Fn1 + 1Fn0)
                problema->feval(tn+h, Yn3, Fn3); // f(tn3,Yn3) -> Fn3

                // Ahora que tenemos Yn3, convertimos los n en n-1 para hacer la siguiente iteracion
                swap(Yn3, Yn2); // Yn3 -> Yn2
                swap(Fn1, Fn0); // Fn1 -> Fn0
                swap(Fn2, Fn1); // Fn2 -> Fn1
                swap(Fn3, Fn2); // Fn3 -> Fn2
                // Tras los cambios, Yn3, Fn3, Y_AB, F_AB, Y_AM & F_AM contienen basura y serán reescritos                
            } // Fin del bucle for principal
        break;
        case 5: // Yn+1 = Yn + h/720 * (251Fn+1 + 646Fn - 264Fn-1 + 106Fn-2 - 19Fn-3)
            for (double tn=t0+h_RK*3.0; tn<tf; tn+=h){
                // Aplicamos Adams-Bashford para obtener una 1ª aproximación de Yn4 -> Y_AB & F_AB
                kernel_sumatoriaAB4<<<get_num_blocks(),get_tam_blocks()>>>(Y_AB, Yn3, Fn3, Fn2, Fn1, Fn0);// Y_AB = Yn3 + h/24 * (55Fn3 - 59Fn2 + 37Fn1 - 9Fn0)
                problema->feval(tn+h, Y_AB, F_AB); // f(tn4, Y_AB) -> F_AB
                
                // Aplicamos Adams-Moulton con F_AB para obtener una 2ª aproximación de Yn4 -> Y_AM & F_AM
                kernel_sumatoriaAM5<<<get_num_blocks(),get_tam_blocks()>>>(Y_AM, Yn3, F_AB, Fn3, Fn2, Fn1, Fn0); // Y_AM = Yn3 + h/720 * (251F_AB + 646Fn3 - 264Fn2 + 106Fn1 - 19Fn0)
                ...
                kernel_sumatoriaAM5<<<get_num_blocks(),get_tam_blocks()>>>(Yn4, Yn3, F_AM, Fn3, Fn2, Fn1, Fn0); // Yn4 = Yn3 + h/720 * (251F_AM + 646Fn3 - 264Fn2 + 106Fn1 - 19Fn0)
                ...
                // Tras los cambios, Yn4, Fn4, Y_AB, F_AB, Y_AM & F_AM contienen basura y serán reescritos                
            } // Fin del bucle for principal
        break;        
