...
__constant__ Params_brusselator2d_shuffle cte4_sh;

// Definicion de los valores constantes para el kernel
void brusselator2d_shuffle::updateConstants() const {
    Params_brusselator2d_shuffle aux;
    ...
    cudaMemcpyToSymbol(cte4_sh, &aux, sizeof(Params_brusselator2d_shuffle));
}

void brusselator2d_shuffle::init(double* Y0) const {
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
...
__global__ void kernel_brusselator2d_shuffle(const double t, const double* __restrict__ Y, double* __restrict__ DY) {
    ...
    if(thread < cte4_sh.neqn){
        ...
        const double term = cte4_sh.B * u_ij - u_ij * u_ij * v_ij;

        DY[thread] = cte4_sh.DD * (val_arriba + val_izq - 4.0 * valor + val_abajo + val_der);
        
        const double x = (id_x + 1) * cte4_sh.dtx, y = (id_y + 1) * cte4_sh.dtx;
        const double xmxc = x - 0.3, ymyc = y - 0.5;
        const double r = 0.1;
        const double result = ((xmxc * xmxc + ymyc * ymyc) <= r * r && t >= 1.1) ? 5.0 : 0.0;

        DY[thread] += id_z==0 ? (cte4_sh.A - term - u_ij + result) : term;
    }
}

void brusselator2d_shuffle::feval(const double &t, const double* Y, double* DY) const {
    kernel_brusselator2d_shuffle<<<this->grid,this->block>>>(t, Y, DY);
}
void brusselator2d_shuffle::feval (const double &h, const double *Y, double* DY, cudaStream_t stream) const {
    kernel_brusselator2d_shuffle<<<this->grid, this->block, 0, stream>>>(0.0, Y, DY);
}
