...
__constant__ Params_RungeKutta cte_RK;
...
void RungeKutta::updateConstants(const int &neqn, const double &h) const {
    ...
    cudaMemcpyToSymbol(cte_RK, &aux, sizeof(Params_RungeKutta));
}
...
__global__ void kernel_sumatoriaRK4(double* __restrict__ Yn, const double* __restrict__ K1, const double* __restrict__ K2, const double* __restrict__ K3, const double* __restrict__ K4) {
    ...
    if(thread<cte_RK.neqn){ // Yn+1 = Yn + h/6 * (K1 + 2*K2 + 2*K3 + K4)
        Yn[thread] += cte_RK.h6 * (K1[thread] + 2.0*K2[thread] + 2.0*K3[thread] + K4[thread]);
    }
}
...
__global__ void kernel_sumatoriaRK3(double* __restrict__ Yn, const double* __restrict__ K1, const double* __restrict__ K2, const double* __restrict__ K3) {
    const int thread = blockDim.x * blockIdx.x + threadIdx.x;
    if(thread<cte_RK.neqn){ // Yn+1 = Yn + h/6 * (K1 + 4*K2 + K3)
        Yn[thread] += cte_RK.h6 * (K1[thread] + 4.0*K2[thread] + K3[thread]);
    }
}

__global__ void kernel_sumatoriaRK2(double* __restrict__ Yn, const double* __restrict__ K1, const double* __restrict__ K2) {
    const int thread = blockDim.x * blockIdx.x + threadIdx.x;
    if(thread<cte_RK.neqn){ // Yn+1 = Yn + h/2 * (K1 + K2)
        Yn[thread] += cte_RK.h2 * (K1[thread] + K2[thread]);
    }
}
...
void RungeKutta::aplicar(Problema* problema, const double &t0, const double &tf, const double &h, const double *Y0, double *Yf) const {
    ...
    cudaMalloc((void**)&Yn,get_bytes());
    cudaMalloc((void**)&K1,get_bytes());
    cudaMalloc((void**)&K2,get_bytes());
    cudaMalloc((void**)&K3,get_bytes());
    cudaMalloc((void**)&K4,get_bytes());
    cudaMalloc((void**)&Yaux,get_bytes());
    ...
    for(double tn=t0; tn<tf; tn+=h){ // Todas las llamadas han de hacerse de forma secuencial, ya que cada una necesita de la anterior
        switch(get_orden()){
            case 1:
                ...
                // Yn+1 = Yn + h*K1
                kernel_escalarPorVector<<<get_num_blocks(),get_tam_blocks()>>>(h, K1, Yn);      // Yn += K1*h
            break;
            case 2:
                // K1 = feval(tn, Yn)
                problema->feval(tn,Yn,K1);              // Definimos K1
                ...
                kernel_escalarSumaMult<<<get_num_blocks(),get_tam_blocks()>>>(Yn, h, K1, Yaux); // Yn + K1*h -> Yaux
                problema->feval(tn + h, Yaux, K2);  // Definimos K2

                // Yn+1 = Yn + h/2 * (K1 + K2)
                kernel_sumatoriaRK2<<<get_num_blocks(),get_tam_blocks()>>>(Yn, K1, K2);
            break;
            case 3:
                // K1 = feval(tn, Yn)
                problema->feval(tn,Yn,K1);              // Definimos K1
                ...
                kernel_escalarSumaMult<<<get_num_blocks(),get_tam_blocks()>>>(Yn, h2, K1, Yaux); // Yaux = Yn + h/2*K1
                problema->feval(tn + h2, Yaux, K2);  // Definimos K2

                // K3 = feval(tn + h, Yn -h*K1 + 2*h*K2 ) 
                kernel_escalarSuma2Mult<<<get_num_blocks(),get_tam_blocks()>>>(Yn, h_n, K1, h_2, K2, Yaux); // Yaux = Yn -h*K1 +2h*K2
                problema->feval(tn + h, Yaux, K3);  // Definimos K3

                // Yn+1 = Yn + h/6 * (K1 + 4*K2 + K3)
                kernel_sumatoriaRK3<<<get_num_blocks(),get_tam_blocks()>>>(Yn, K1, K2, K3);
            break;
            case 4:
                // K1 = feval(tn, Yn)
                problema->feval(tn,Yn,K1);              // Definimos K1
                ...
                kernel_escalarSumaMult<<<get_num_blocks(),get_tam_blocks()>>>(Yn, h2, K2, Yaux); // Yaux = Yn + h/2*K2
                problema->feval(tn + h2, Yaux, K3);  // Definimos K3

                // K4 = feval(tn + h, Y0 + h*K3)
                kernel_escalarSumaMult<<<get_num_blocks(),get_tam_blocks()>>>(Yn, h, K3, Yaux); // Yaux = Yn + h*K3
                problema->feval(tn+h, Yaux, K4);        // Definimos K4

                // Yn+1 = Yn + h/6 * (K1 + 2*K2 + 2*K3 + K4)
                kernel_sumatoriaRK4<<<get_num_blocks(),get_tam_blocks()>>>(Yn, K1, K2, K3, K4);
            break;        // Tras las sumas, el vector Yn ahora es Yn+1
        } // Fin del switch
    } // Fin del bucle for

    // Yn es el valor que arrastramos de la ultima iteracion
    cudaMemcpy(Yf, Yn, get_bytes(), cudaMemcpyDeviceToHost); // Yn -> Yf
    cudaFree(Yn);
    cudaFree(K1);
    cudaFree(K2);
    cudaFree(K3);
...
void RungeKutta::aplicarUnidad(Problema* problema, const double &t0, const double &h, const double *Y0, double *Yf) const {
    ...
    cudaMemcpy(Yn, Y0, get_bytes(), cudaMemcpyHostToDevice); // Y0 -> Yn
