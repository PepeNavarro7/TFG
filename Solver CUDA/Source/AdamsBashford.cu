    ...
    switch(get_orden()){ // Switch principal con el for que se trabaja
        case 1: 
            for(double tn = t0; tn<tf; tn+=h){
                kernel_sumatoriaAB1<<<get_num_blocks(),get_tam_blocks()>>>(Yn1, Yn0, Fn0); // Yn1 = Yn0 + h * Fn0
                problema->feval(tn+h, Yn1, Fn1); // f(tn1,Yn1) -> Fn1
                
                // Ahora que tenemos Yn1 y Fn1, convertimos todos los n en n-1
                swap(Yn1, Yn0); // Yn1 -> Yn0
                swap(Fn1, Fn0); // Fn1 -> Fn0
                // Tras los cambios, Yn1 y Fn1 contienen basura y serán reescritos
            } // Fin del bucle for iterativo
        break;
        case 2:
            for(double tn = t0+h_RK*1.0; tn<tf; tn+=h){
                kernel_sumatoriaAB2<<<get_num_blocks(),get_tam_blocks()>>>(Yn2, Yn1, Fn1, Fn0); // Yn2 = Yn1 + h/2 * (3*Fn1 - Fn0)
                problema->feval(tn+h, Yn2, Fn2); // f(tn2,Yn2) -> Fn2
                
                // Ahora que tenemos Yn2 y Fn2, convertimos los n en n-1 para hacer la siguiente iteracion
        ...
        case 3:
            for(double tn = t0+h_RK*2.0; tn<tf; tn+=h){
                kernel_sumatoriaAB3<<<get_num_blocks(),get_tam_blocks()>>>(Yn3, Yn2, Fn2, Fn1, Fn0); // Yn3 = Yn2 + h/12 * (23*Fn2 - 16*Fn1 + 5*Fn0)
                problema->feval(tn+h, Yn3, Fn3); // f(tn3,Yn3) -> Fn3

                // Ahora que tenemos Yn3 & Fn3, convertimos los n en n-1 para hacer la siguiente iteracion
        ...
        case 4:
            for(double tn = t0+h_RK*3.0; tn<tf; tn+=h){
                ...
                kernel_sumatoriaAB4<<<get_num_blocks(),get_tam_blocks()>>>(Yn4, Yn3, Fn3, Fn2, Fn1, Fn0); // Yn4 = Yn3 + h/24 * (55*Fn3 - 59*Fn2 + 37*Fn1 - 9*Fn0)
                problema->feval(tn+h, Yn4, Fn4); // f(tn4,Yn4) -> Fn4
                
                // Ahora que tenemos Yn4 & Fn4, n en n-1 para hacer la siguiente iteracion
